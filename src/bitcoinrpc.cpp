...
Value stop(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "stop\n"
            "Stop florijncoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "florijncoin server has now stopped running!";
}
...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    GenerateFlorijnCoins(fGenerate, pwalletMain);
    return Value::null;
}
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new florijncoin address for receiving payments.  "
    ...
    return CFlorijnCoinAddress(keyID).ToString();
}


CFlorijnCoinAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CFlorijnCoinAddress(account.vchPubKey.GetID());
}
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current florijncoin address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <florijncoin address> <account>\n"
            "Sets the account associated with the given address.");

    CFlorijnCoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid florijncoin address");
...
Value getaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getaccount <florijncoin address>\n"
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    ...
    Array ret;
    BOOST_FOREACH(const PAIRTYPE(CFlorijnCoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CFlorijnCoinAddress& address = item.first;
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress <florijncoin address> <amount> [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.00000001"
            + HelpRequiringPassphrase());
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <florijncoin address> <message>\n"
    ...
    CFlorijnCoinAddress addr(strAddress);
    if (!addr.IsValid())
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <florijncoin address> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <florijncoin address> [minconf=1]\n"
            "Returns the total amount received by <florijncoin address> in transactions with at least [minconf] confirmations.");

    // florijncoin address
    CFlorijnCoinAddress address = CFlorijnCoinAddress(params[0].get_str());
    CScript scriptPubKey;
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid florijncoin address");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom <fromaccount> <to florijncoin address> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.00000001"
            + HelpRequiringPassphrase());

    string strAccount = AccountFromValue(params[0]);
    CFlorijnCoinAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid florijncoin address");
    int64 nAmount = AmountFromValue(params[2]);
    int nMinDepth = 1;
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    set<CFlorijnCoinAddress> setAddress;
    vector<pair<CScript, int64> > vecSend;

    int64 totalAmount = 0;
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        CFlorijnCoinAddress address(s.name_);
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid florijncoin address:")+s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress <nrequired> <'[\"key\",\"key\"]'> [account]\n"
            ...
            "each key is a florijncoin address or hex-encoded public key\n"
    ...
    for (unsigned int i = 0; i < keys.size(); i++)
    {
        ...
        // Case 1: florijncoin address and we have full public key:
        CFlorijnCoinAddress address(ks);
        if (address.IsValid())
        {
            CKeyID keyID;
    ...
    return CFlorijnCoinAddress(innerID).ToString();
}
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<CFlorijnCoinAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CFlorijnCoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CFlorijnCoinAddress, tallyitem>::iterator it = mapTally.find(address);
...
void ListTransactions(const CWalletTx& wtx, const string& strAccount, int nMinDepth, bool fLong, Array& ret)
{
    ...
    if ((!listSent.empty() || nFee != 0) && (fAllAccounts || strAccount == strSentAccount))
    {
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64)& s, listSent)
        {
            ...
            entry.push_back(Pair("address", CFlorijnCoinAddress(s.first).ToString()));
    ...
    if (listReceived.size() > 0 && wtx.GetDepthInMainChain() >= nMinDepth)
    {
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64)& r, listReceived)
        {
            ...
            if (fAllAccounts || (account == strAccount))
            {
                ...
                entry.push_back(Pair("address", CFlorijnCoinAddress(r.first).ToString()));
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; florijncoin server stopping, restart to run with encrypted wallet";
}
...
public:
    ...
    Object operator()(const CScriptID &scriptID) const {
        ...
        BOOST_FOREACH(const CTxDestination& addr, addresses)
            a.push_back(CFlorijnCoinAddress(addr).ToString());
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <florijncoin address>\n"
            "Return information about <florijncoin address>.");
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "florijncoin server is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "florijncoin server is downloading blocks...");
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: florijncoin-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
//
// JSON-RPC protocol.  florijncoin speaks version 1.0 for maximum compatibility,
...
void ThreadRPCServer2(void* parg)
{
    ...
    if (mapArgs["-rpcpassword"] == "")
    {
        ...
        string strWhatAmI = "To use florijncoin";
