...
static const char *strDNSSeed[][2] = {
    {"VPS Node", "vps382756.ovh.net ( ip 164.132.45.38)"},
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. florijncoin is probably already running."), addrBind.ToString().c_str());
        else
...
void StartNode(void* parg)
{
    ...
    GenerateFlorijnCoins(GetBoolArg("-gen", false), pwalletMain);
}
...
bool StopNode()
{
    ...
    if (vnThreadsRunning[THREAD_MINER] > 0) printf("ThreadFlorijnCoinMiner still running\n");
