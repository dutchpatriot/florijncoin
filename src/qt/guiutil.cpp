...
bool parseFlorijnCoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    //
    //    Cannot handle this later, because florijncoin:// will cause Qt to see the part after // as host,
    //    which will lowercase it (and thus invalidate the address).
    if(uri.startsWith("florijncoin://"))
    {
                                 ...
                                 QString *selectedSuffixOut)
{
    ...
    /* Extract first suffix from filter pattern "Description (*.FLC)" or "Description (*.FLC *.bar ...) */
    QRegExp filter_re(".* \\(\\*\\.(.*)[ \\)]");
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "florijncoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for florijncoin.lnk
...
{
    return GetAutostartDir() / "florijncoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a florijncoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=florijncoin\n";
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("florijncoin-qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  florijncoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("florijncoin-qt"));
